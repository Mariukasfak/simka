...
export async function GET(request: Request) {
  try {
    ...
    // ⚡ Bolt Performance Optimization: Parallelized independent Supabase queries and optimized payload size
    // Using Promise.all reduces total API latency, and replacing select('*') with specific columns saves memory/bandwidth
    const [
      { data: orders, error: ordersError },
      { data: popularProducts, error: productsError },
      { data: dailyRevenue, error: revenueError },
    ] = await Promise.all([
      // Get all orders (optimized to only fetch needed fields)
      supabase
        .from("orders")
