...
export async function GET(request: Request) {
  try {
    ...
    // Fetch GitHub user data and current Supabase session in parallel to optimize latency
    const [githubUser, sessionResponse] = await Promise.all([
    ...
    const { data: user, error: userError } = await supabase
      ...
      .eq("id", sessionResponse.data.user?.id)
      .select()
      .single();
