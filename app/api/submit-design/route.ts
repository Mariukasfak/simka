...
async function saveOrderToDb(data: any, validatedData: any) {
  try {
    let userId = null;
...
async function sendOrderEmail(data: any, validatedData: any, emailHtml: string) {
  try {
    ...
    await transporter.sendMail({
      from: `"Siemka Design Tool" <${process.env.EMAIL_FROM || process.env.EMAIL_USER}>`,
      to: recipientEmail,
...
export async function POST(request: Request) {
  try {
    ...
    // Vykdome operacijas lygiagrečiai
    // Use IIFEs or simply call the async functions. Calling them directly creates promises.
    await Promise.allSettled([
      saveOrderToDb(data, validatedData),
      sendOrderEmail(data, validatedData, emailHtml)
    ]);
