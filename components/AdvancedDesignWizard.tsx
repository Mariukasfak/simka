...
interface AdvancedDesignWizardProps {
  ...
  printAreas: {
    ...
    'left-sleeve'?: { top: number; left: number; width: number; height: number }
    'right-sleeve'?: { top: number; left: number; width: number; height: number }
  }
  onComplete: (designs: {
    area: string
    preview: string
...
export default function AdvancedDesignWizard({
  ...
  const [designStates, setDesignStates] = useState<Record<string, {
    position: DesignPosition
    relativePosition: RelativePosition
    scale: number
    opacity: number
  ...
  const handleSaveDesign = useCallback((state: {
    position: DesignPosition 
    relativePosition: RelativePosition
    scale: number
    opacity: number
    ...
    // Užrakiname dizainą
    setDesignStates(prev => ({ ...prev, [currentView]: { ...state, locked: true } }))
    
    // Tikrinti ar turime daugiau sričių, į kurias galime pereiti
    const availableViews = Object.keys(productImages || {}).filter(view => view !== currentView)
  ...
  if (!productImages || !printAreas) {
    return (
      <div className="p-6 bg-red-50 rounded-lg border border-red-200 text-red-800">
        ...
        <p>Dizaino vedliui trūksta reikalingų duomenų: produktų vaizdų arba spausdinimo zonų.</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-white rounded-lg shadow-md p-4 sm:p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">Dizaino redagavimas</h2>
        <p className="text-gray-600">Pridėkite ir koreguokite savo logotipą ant produkto</p>
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-6">
        {Object.entries(productImages).map(([view, _]) => (
          ...
          >
            {view === 'front' ? 'Priekis' : 
             view === 'back' ? 'Nugara' : 
             view === 'left-sleeve' ? 'Kairė rankovė' : 
             view === 'right-sleeve' ? 'Dešinė rankovė' : 
             view}
          </Button>
        ))}
      </div>
      ...
      {uploadedImages[currentView] ? (
        ...
        <SimplifiedDesignCanvas
          productImage={productImages[currentView as keyof typeof productImages] || ''}
          uploadedImage={uploadedImages[currentView]}
          initialState={designStates[currentView] || undefined}
          printAreaBounds={printAreas[currentView as keyof typeof printAreas] || { top: 25, left: 25, width: 50, height: 50 }}
        ...
        <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-700 mb-2">Įkelkite savo logotipą</h3>
          <p className="text-gray-500 mb-4">PNG, JPG arba SVG formatas (rekomenduojama skaidri PNG)</p>
          <Button
            onClick={handleAddDesignToCurrentView}
            icon={ImageIcon}
          >
      ...
      {Object.values(previews).some(preview => preview !== null) && (
        <div className="mt-8">
          <h3 className="text-lg font-medium text-gray-700 mb-4">Jūsų dizainai</h3>
              ...
              preview && (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white text-xs p-1 text-center capitalize">
                    {view === 'front' ? 'Priekis' : 
                     view === 'back' ? 'Nugara' : 
                     view === 'left-sleeve' ? 'Kairė' : 
                     view === 'right-sleeve' ? 'Dešinė' : 
                     view}
                  </div>
                </div>
              )
      ...
      <div className="mt-8 flex justify-between">
        ...
        <Button
          ...
          disabled={!Object.values(designStates).some(state => state !== null && state.locked)}
          className="ml-auto"
          variant="default"
        >
          Užbaigti dizainą
