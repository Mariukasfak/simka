...
export default function DesignDebugger({
  ...
  const calculateRelativePositionInPrintArea = () => {
    if (!containerRect || !printAreaRect || !designState)
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-black bg-opacity-80 text-white p-2 text-xs z-50">
      <div className="flex flex-wrap gap-2">
        <div>
          <b>Pozicija (px):</b> X: {Math.round(designState?.position?.x || 0)},
          Y: {Math.round(designState?.position?.y || 0)}
        </div>
        <div>
          <b>Santykis printArea (%):</b> X: {Math.round(relativePos.xPercent)}%,
          Y: {Math.round(relativePos.yPercent)}%
        </div>
        <div>
          <b>Elementų dydžiai:</b> Container: {containerRect?.width.toFixed(0)}x
          {containerRect?.height.toFixed(0)}px, PrintArea:{" "}
          {printAreaRect?.width.toFixed(0)}x{printAreaRect?.height.toFixed(0)}px
        </div>
        <div>
          <b>PrintArea (CSS):</b> L: {printArea?.bounds?.left}%, T:{" "}
          {printArea?.bounds?.top}%, W: {printArea?.bounds?.width}%, H:{" "}
          {printArea?.bounds?.height}%
        </div>
        <div>
          <b>Skalė:</b> {designState?.scale.toFixed(2)}, <b>Pasukimas:</b>{" "}
