...
export default function EnhancedDesignCanvas({
  ...
  const [dragRelativePosition, setDragRelativePosition] = useState(
  ...
  const generatePreview = useCallback(
    debounce(async (forceGenerate = false) => {
      ...
      try {
        ...
        const originalLogo = originalCanvas.querySelector(
          ".draggable-image",
        ) as HTMLElement;
        const previewLogo = previewContainer.querySelector(
          ".draggable-image",
        ) as HTMLElement;
        ...
        const canvas = await html2canvas(previewContainer, {
          ...
          onclone: (clonedDoc, clonedElem) => {
            const clonedLogo = clonedElem.querySelector(
              ".draggable-image",
            ) as HTMLElement;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between text-sm text-gray-500">
        <span>X: {Math.round(dragPosition.x)}</span>{" "}
        {/* ⚡ Using local state */}
        <span>Y: {Math.round(dragPosition.y)}</span>{" "}
        {/* ⚡ Using local state */}
      ...
      {process.env.NODE_ENV === "development" &&
        dragRelativePosition /* ⚡ Using local state */ && (
          ...
          <div className="bg-white rounded-lg p-6 max-w-lg mx-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                ...
                className="text-gray-500 hover:text-gray-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500 rounded"
              >
