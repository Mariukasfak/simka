...
export default function ProductSelector({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4" role="group" aria-label="Produktų pasirinkimas">
        {products.map((product) => (
          <motion.button
            key={product.id}
            onClick={() => onSelect(product)}
            aria-pressed={selectedProduct.id === product.id}
            className={`relative p-4 rounded-lg border-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500 focus-visible:ring-offset-2 ${
