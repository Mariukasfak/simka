...
interface RelativePositionDraggableImageProps {
  ...
  onRelativePositionChange?: (position: {
    xPercent: number;
    yPercent: number;
  }) => void;
  onPositionChangeEnd?: (position: DesignPosition) => void;
  onRelativePositionChangeEnd?: (position: {
    xPercent: number;
    yPercent: number;
  }) => void;
...
export default function RelativePositionDraggableImage({
  ...
  const currentRelativePositionRef = useRef(
  ...
  const containerDimensionsRef = useRef<{ width: number; height: number }>({
    width: 0,
    height: 0,
  });
  ...
  const relativeToAbsolutePosition = useCallback(
    (relPos: { xPercent: number; yPercent: number }): DesignPosition => {
  ...
  const absoluteToRelativePosition = useCallback(
    (absPos: DesignPosition): { xPercent: number; yPercent: number } => {
  ...
  const updateBounds = useCallback(() => {
    ...
    if (printAreaRef?.current) {
      ...
      setBounds({
        ...
        right: Math.min(
          100,
          ((printAreaLeft + printArea.width + margin) / container.width) * 100,
        ),
        bottom: Math.min(
          100,
          ((printAreaTop + printArea.height + margin) / container.height) * 100,
        ),
      });
    } else {
  ...
  const updateElementPosition = useCallback(
    (
      ...
      relPos: { xPercent: number; yPercent: number },
    ) => {
  ...
  const updatePositionFromRelative = useCallback(
    (xPercent: number, yPercent: number) => {
      // Ribojame santykinę poziciją
      const boundedXPercent = Math.max(
        bounds.left,
        Math.min(bounds.right, xPercent),
      );
      const boundedYPercent = Math.max(
        bounds.top,
        Math.min(bounds.bottom, yPercent),
      );

      // Konvertuojame į absoliučią poziciją
      const absPos = relativeToAbsolutePosition({
        xPercent: boundedXPercent,
        yPercent: boundedYPercent,
      });

      // Atnaujiname elemento poziciją
      updateElementPosition(absPos, {
        xPercent: boundedXPercent,
        yPercent: boundedYPercent,
      });
    },
  ...
  const updatePositionFromAbsolute = useCallback(
    (x: number, y: number) => {
      ...
      const boundedXPercent = Math.max(
        bounds.left,
        Math.min(bounds.right, relPos.xPercent),
      );
      const boundedYPercent = Math.max(
        bounds.top,
        Math.min(bounds.bottom, relPos.yPercent),
      );

      // Konvertuojame atgal į absoliučią poziciją
      const boundedAbsPos = relativeToAbsolutePosition({
        xPercent: boundedXPercent,
        yPercent: boundedYPercent,
      });

      // Atnaujiname elemento poziciją
      updateElementPosition(boundedAbsPos, {
        xPercent: boundedXPercent,
        yPercent: boundedYPercent,
      });
    },
  ...
  useEffect(() => {
    ...
    window.addEventListener("resize", () => {
      ...
      if (currentRelativePositionRef.current) {
        const absPos = relativeToAbsolutePosition(
          currentRelativePositionRef.current,
        );
  ...
  useEffect(() => {
    ...
    if (!isDragging) {
      ...
      if (!relativePosition) {
        currentRelativePositionRef.current =
          absoluteToRelativePosition(position);
      }
  ...
  useEffect(() => {
    ...
    if (
      currentRelativePositionRef.current.xPercent ===
        relativePosition.xPercent &&
      currentRelativePositionRef.current.yPercent ===
        relativePosition.yPercent &&
      !isDragging
    ) {
      return;
    }
