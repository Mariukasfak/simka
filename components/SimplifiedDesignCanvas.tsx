  ...
  const generatePreview = useCallback(
    debounce((forceGenerate = false) => {
      if (
        !canvasRef.current ||
        !uploadedImage ||
        (generatingRef.current && !forceGenerate)
      ) {
        return;
      }
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2 justify-between bg-white rounded-lg p-3 shadow-sm">
        ...
        {locked && (
          <Button variant="default" size="sm" onClick={handleSave} icon={Check}>
            Išsaugoti
          </Button>
        )}
      </div>
      ...
      <div className="text-sm text-gray-500 text-center">
        {locked ? (
          <p>
            Dizainas užrakintas. Atlikus pakeitimus, paspauskite „Išsaugoti".
          </p>
        ) : (
          <p>
            Tempkite logotipą, koreguokite dydį ir pasukimą. Tada paspauskite
            „Užrakinti" mygtuką.
          </p>
        )}
      </div>
