...
export function useDesignCollection() {
  ...
  const removeDesign = useCallback((designId: string) => {
    setState(prev => {
      ...
      Object.keys(newDesigns).forEach(area => {
        newDesigns[area as PrintAreaPosition] = newDesigns[area as PrintAreaPosition]
      ...
      return {
        ...prev,
        designs: newDesigns,
        activeDesignId: prev.activeDesignId === designId ? null : prev.activeDesignId
      }
    })
  }, [])
  ...
  const updatePreview = useCallback((area: PrintAreaPosition, preview: string | null) => {
    setState(prev => ({
      ...prev,
      previews: {
        ...prev.previews,
  ...
  const getAreaStatus = useCallback((area: PrintAreaPosition) => {
  ...
  const getTotalDesignsCount = useCallback(() => {
    return Object.values(state.designs).reduce((total, designs) => total + designs.length, 0)
  }, [state.designs])
  ...
  return {
    ...
    setActiveDesignId: (id: string | null) => setState(prev => ({ ...prev, activeDesignId: id })),
    setCurrentArea,
    addDesign,
    updateDesign,
    removeDesign,
